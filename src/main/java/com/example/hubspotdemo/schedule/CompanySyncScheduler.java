...
public class CompanySyncScheduler {
    ...
    public void performIncrementalSync() {
        ...
        try {
            ...
            Map<String, Object> filter = buildIncrementalFilter();
            ...
            if (response != null && response.getResults() != null) {
                ...
                for (Company company : response.getResults()) {
                    logger.debug("处理更新的公司数据: ID={}, Name={}", company.getId(), company.getProperties().get("companyname"));
                }
            }
        } catch (Exception e) {
     ...
     * @return 过滤条件对象
     */
    private Map<String, Object> buildIncrementalFilter() {
        // 这里构建的是一个简单的过滤条件示例
        ...
        filter.put("propertyName", "lastmodifieddate");
        filter.put("operator", "GT"); // GT = Greater Than
        
        // 计算5分钟前的时间戳（毫秒）
        long fiveMinutesAgo = System.currentTimeMillis() - (5 * 60 * 1000);
        filter.put("value", fiveMinutesAgo);
        
        filterGroup.put("filters", Collections.singletonList(filter));
        
        Map<String, Object> filterGroups = new HashMap<>();
        filterGroups.put("filterGroups", Collections.singletonList(filterGroup));
